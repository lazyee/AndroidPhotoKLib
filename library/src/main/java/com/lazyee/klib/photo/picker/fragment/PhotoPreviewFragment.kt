...
import com.lazyee.klib.photo.extension.scrollPositionToCenter
...
internal class PhotoPreviewFragment : Fragment(),OnBigImageClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.vpPhotos.addOnPageChangeListener(object : OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                ...
                binding.rvPhotos.scrollPositionToCenter(position, 200)
            }
    ...
    fun getRemovePhotoList(): MutableList<Photo> {
        return removePhotoList
    }
    ...
    private val topAnimIn by lazy {
        AnimationUtils.loadAnimation(activity, R.anim.anim_preview_top_in).also {
    ...
    private val topAnimOut by lazy {
        AnimationUtils.loadAnimation(activity, R.anim.anim_preview_top_out).also {
            it.setAnimationListener(object : Animation.AnimationListener {
                override fun onAnimationStart(animation: Animation?) {
                    isAnimPlaying = true
                }
    ...
    private val bottomAnimIn by lazy {
        AnimationUtils.loadAnimation(
            activity,
            R.anim.anim_preview_bottom_in
        )
    }
    private val bottomAnimOut by lazy {
        AnimationUtils.loadAnimation(
            activity,
            R.anim.anim_preview_bottom_out
        )
    }
    ...
    //选中的图片的适配器
    inner class SelectedPhotoAdapter(private val photos: List<Photo>) :
        RecyclerView.Adapter<SelectedPhotoViewHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SelectedPhotoViewHolder {
            return SelectedPhotoViewHolder(
                TemplatePreviewSelectedPhotoBinding.inflate(
        ...
        override fun onBindViewHolder(holder: SelectedPhotoViewHolder, position: Int) {
            ...
            holder.binding.llCurrent.visibility =
                if (position == binding.vpPhotos.currentItem) View.VISIBLE else View.GONE
        }
    ...
    inner class SelectedPhotoViewHolder(val binding: TemplatePreviewSelectedPhotoBinding) :
